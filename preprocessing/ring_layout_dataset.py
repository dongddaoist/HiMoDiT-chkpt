"""
HiMoFlow v5.4 — Ring layout dataset extraction (Step 2).

Extracts (R, F, L, P_len, P_pos, atom_ids) labels from each RedDB SMILES
for A1 + A2 training. Performs terminal stripping using v5.3's locked
SMARTS list (extended in v5.4 with =O/=NH/=S double-bond terminals)
to identify what's NOT scaffold. Rejects molecules whose scaffold
doesn't fit v5.4-1's vocab:
  - Rings must be size 5 or 6
  - Ring graph must be a tree (no peri fusion)
  - Multi-ring chains must be LINEAR fusion (no phenanthrene-style angular)
  - Maximum R_MAX rings, M_MAX atoms

Pipeline per molecule:
  1. Parse SMILES with RDKit, sanitize
  2. Detect terminals (parent matches only after subset removal) using
     v5.3's algorithm — see preprocessing/detect_terminals_v3.py
  3. Identify scaffold = atoms NOT in any detected terminal's atom_indices
  4. Find rings of the scaffold via SSSR, but filter to only those rings
     whose atoms are all in the scaffold (a ring partly in a terminal
     is impossible because terminals have only 1 crossing bond)
  5. Classify topology: each scaffold ring pair is fused, linked, or unrelated
  6. Reject if topology violates v5.4-1 rules
  7. Determine canonical ring ordering: BFS from anchor (ring with smallest
     atom-index minimum); ties broken by ring index
  8. Extract canonical ring traversals matching the decoder's
     "anchor edge at (sz-1, 0), far edge opposite" convention
  9. Collect non-ring scaffold atoms as pendants attached to rings
 10. Validate via round-trip: decode the extracted layout, compare
     bond classes against the scaffold portion of the original molecule

Outputs cached to ring_layout_labels.pkl as a list of dicts with the
v5.4-1 layout label PLUS the list of detected terminals (for visibility
and debugging; A1+A2 don't use them, but this lets us pair the labels
with Stage 2's terminal-vocab output later).
"""
...
from meanflow.ring_layout_decoder import (
    ...
    RING_7_AROM, RING_7_ALIPH,  # v5.4-ZINC Phase 2B applied
...
# Import terminal detection (v5.3 algorithm + v5.4 extended SMARTS)
# v5.4-ZINC Phase 2D applied: redirect to K=22 vocab with charged-form fixes
from preprocessing.terminal_smarts_v5_4_zinc import CURATED_TERMINALS
...
# ─── v5.3 atom vocab (matches config.py) ────────────────────────────
# {0:<PAD>, 1:c, 2:O, 3:C, 4:N, 5:n, 6:S, 7:F, 8:s, 9:o}
# v5.4-ZINC Phase 2D applied. ATOM_VOCAB 10 -> 16 with charged classes.
...
def _rdkit_atom_to_vocab_id(atom: Chem.Atom) -> Optional[int]:
    """v5.4-ZINC Phase 2D applied. Charge-aware vocab mapping.
...
def _check_topology(
    ...
    """Validate that the scaffold ring system fits v5.4-1's vocab.
    ...
    for i, r in enumerate(rings):
        if len(r) not in (3, 4, 5, 6, 7):  # v5.4-ZINC Phase 2B applied
...
def _ring_type_id(ring_size_n: int, is_aromatic: bool) -> int:
    """Map (size, aromatic) to vocab ID. v5.4-ZINC Phase 2B applied."""
...
def extract_layout(
    ...
    Performs terminal stripping using v5.3's SMARTS list (extended in v5.4
    ...
    # Build atom_ids using v5.3 vocab
    ...
    try:
        aip, bc, am = decode_layout_to_scaffold(R, F, L, P_len, P_pos, atom_ids)
...
def print_rejection_examples(
    ...
    Call after extract_dataset(... collect_rejection_examples=N) to see
...
# ────────────────────────────────────────────────────────────────────────
# v5.5 unified encoder: extract_layout_v5_5
# ────────────────────────────────────────────────────────────────────────
#
# Drop-in sibling of extract_layout() with two architectural extensions:
...
#   v5.4 baseline:         83.14% retention
#   v5.5 unified encoder:  93.25% retention  (+10.11 pp, 0 regressions)
#
# Label schema additions over v5.4:
...
#
# Legacy linear-pendant fields (P_len, P_pos) are NOT emitted by v5.5 —
# the branch-tree representation supersedes them. For linear pendants,
# B_size==1 is the natural single-atom branch.
#
# This function is the source of truth for v5.5 preprocessing.
...
# v5.5 capacities
V5_5_P_MAX = 8        # max branches per ring (v5.4 was 6 linear pendants)
V5_5_B_LEN_MAX = _B_LEN_MAX  # max atoms per branch tree

# Bond class codes (same as v5.4 build_bond_classes output)
_BOND_NONE     = 0
_BOND_SINGLE   = 1
_BOND_AROMATIC = 2
_BOND_DOUBLE   = 3
...
def _rdkit_bond_class_v5_5(bond):
...
def _is_valid_spiro_center_v5_5(atom):
...
def _classify_ring_pair_v5_5(m, ring_a_idx, ring_b_idx, all_rings):
    """v5.5 ring-pair classifier: accepts spiro at valid centers, delegates rest to v5.4."""
    ring_a = all_rings[ring_a_idx]
    ring_b = all_rings[ring_b_idx]
    ...
    if len(shared) == 1:
        ...
        is_valid, reason = _is_valid_spiro_center_v5_5(atom)
        if is_valid:
...
def _check_topology_v5_5(m, rings):
    """v5.5 topology check: spiro counts as ring-graph edge (chains allowed)."""
    n = len(rings)
    for i, r in enumerate(rings):
    ...
    for i in range(n):
        for j in range(i + 1, n):
            rel, info = _classify_ring_pair_v5_5(m, i, j, rings)
            relations[(i, j)] = (rel, info)
            if rel in ("spiro_invalid", "peri_invalid", "no_clean_endpoint"):
                return ("rejected", f"rings_{i}_{j}_{rel}")
    ...
    # Linear-fusion check (unchanged from v5.4, FUSED edges only)
    for ring_idx in range(n):
        fused_neighbors = []
        for nbr in G.neighbors(ring_idx):
            if G.edges[ring_idx, nbr]["relation"] == "fused":
...
def _extract_branch_trees_v5_5(m, R_MAX_, P_MAX_, B_LEN_MAX_,
        ...
        for trav_pos, ring_atom_rdkit in enumerate(tv):
            ...
            for nbr in atom.GetNeighbors():
                ...
                while queue:
                    ...
                    bcls = _rdkit_bond_class_v5_5(bond)
                    if bcls is None:
...
def extract_layout_v5_5(
    smi: str,
    P_MAX_param: int = V5_5_P_MAX,
    B_LEN_MAX_param: int = V5_5_B_LEN_MAX,
) -> Tuple[Optional[Dict], Optional[str]]:
    """v5.5 unified encoder: branched pendants + spiro F_SPIRO=3.

    Validated on full ZINC250K: 93.25% retention vs 83.14% v5.4 baseline,
    0 regressions, +10.11 pp absolute.

    Returns (label_dict, None) on success or (None, reject_reason) on failure.
    The label dict contains all v5.4 fields except (P_len, P_pos), plus:
      B_size, B_pos, B_parent, B_bond, spiro_atom_positions
    """
    m = Chem.MolFromSmiles(smi)
    if m is None:
        return None, "smiles_parse_failed"

    # Terminal detection (same as v5.4)
    ...
    status, info = _check_topology_v5_5(m, rings)
    if status == "rejected":
        return None, info
    ...
    # Branched pendants
    branch_result, branch_err = _extract_branch_trees_v5_5(
    ...
    # Canonical layout (v5.4-style explicit, plus spiro handling)
    canonical_to_rdkit = {}
    next_canonical = 0
    # Ring 0 fully
