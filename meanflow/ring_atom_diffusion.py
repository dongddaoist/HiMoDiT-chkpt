"""
HiMoFlow v5.4 — Ring Atom Diffusion (A2, Batch 4).
====================================================

Discrete absorbing-state diffusion over scaffold atom IDs, conditioned on
(a) the clean bond skeleton produced by the decoder (which composes A1's
layout into v5.3-compatible scaffold tensors), and (b) the property
condition (solubility_norm, gap_norm).
...
-----------------
   (sol, gap)
       │
       ▼
   A1 (Batch 3) ───► layout (R, F, L, P_len, P_pos)
                            │
                            ▼
              decode_layout_to_scaffold (deterministic)
                            │
                            ▼
            (atom_ids_pad=0,        ◄── A2's job: predict these
             bond_classes,          ◄── given as input (clean)
             atom_mask,
             aromatic_constraint_mask)
                            │
                            ▼
       v5.3 Stage 2 (terminals) — Batch 5

A2 takes the bond skeleton as a known graph (no denoising on bonds — that
was v5.3's joint-denoising failure mode) and only denoises atom IDs
through a graph DiT with edge-biased attention.

Architecture
------------
- One token per atom slot (T = M_MAX = 24).
- Each token's embedding combines a learnable positional embedding and a
  learnable atom-value embedding (the value embed has +1 MASK class on
  the input side; output head predicts over the no-MASK vocab).
- DiT blocks with EdgeBiasedMultiheadAttention: attention scores between
  tokens i,j are biased by per-head learned weights over the (clean)
  bond class at edge (i,j). Padding atoms are masked at attention via
  `key_padding_mask`. AdaLN modulation from time + condition.
- Output head: linear → N_ATOM_CLASSES logits per token.
- Aromatic constraint (atoms in aromatic rings must take aromatic IDs)
  is enforced at sampling time via logit masking. At training time we
  rely on the data — since the dataset only contains valid atom/ring
  combinations, the model learns the constraint from examples; the
  hard mask at sampling guarantees it for OOD layouts.

Loss: CE on atom logits at every VALID position (uses atom_mask). Pad
positions contribute zero loss (matches v5.3 stage-2 convention for
masked-loss). This differs from A1, where every position participated
in CE — A2 has explicit padding because scaffold sizes vary per molecule.

Sampling: iterative confidence-based unmasking with per-row top-K and
Gumbel perturbation, identical to A1's sampler in spirit. At every
step, atomic logits are masked according to:
   - PAD positions (atom_mask=False) forced to ATOM_PAD class.
   - Aromatic-required positions: aliphatic atom IDs → logit=-inf.
   - Aliphatic-required positions: PAD-only ATOM_PAD is excluded; the
     other 5 classes (1..9 minus aromatic) are all viable.
   - The model's own MASK is never produced (handled by predicting
     over the no-MASK vocab).
"""
...
from .edge_biased_attention import EdgeBiasedMultiheadAttention
...
ATOM_PAD = 0
# v5.4-ZINC Phase 2D applied. Atom-vocab 10 -> 16 with charged classes.
...
from meanflow.ring_layout_decoder import M_MAX  # noqa: E402,F401
...
class AdaLN(nn.Module):
    ...
    def __init__(self, d_model: int, d_cond: int):
        ...
        self.proj = nn.Sequential(
            nn.SiLU(), nn.Linear(d_cond, 2 * d_model),
        )
...
class GraphDiTBlock(nn.Module):
    ...
    Mirrors v5.3's `dit_matrix.py` GraphDiT block but without cross-
...
class RingAtomDiffusion(nn.Module):
...
def build_ring_atom_diffusion(
    ...
    **overrides,
    ...
    N_BOND_CLASSES=3 (v5.3/v5.4 RedDB vocab: none/single/aromatic).
    Set to 5 for v5.5 (adds double, triple — produced by the v5.5
