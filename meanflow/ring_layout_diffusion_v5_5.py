"""
HiMoFlow v5.5 — Ring Layout Diffusion (A1) for ZINC250K.
========================================================

Discrete absorbing-state diffusion over the v5.5 layout state
(R, F, L, spiro_pos_class). Pendant prediction has moved to A3 (the
branch-topology diffusion stage); A1 now predicts only the ring
skeleton + ring relations + per-pair spiro position.

Differences vs v5.4 (`meanflow/ring_layout_diffusion.py`):

- Imports M_MAX, R_MAX, P_MAX, L_MAX, B_LEN_MAX, F_*, RING_TYPE_INFO
  from `meanflow.ring_layout_decoder` instead of redefining them.
  Single source of truth.
- R_MAX 4 → 6 (Phase 2A)
- L_MAX 4 → 10 (Phase 2A)
- N_R_CLASSES 5 → 11 (PAD + 10 ring types; Phase 2B added 3/4/7-mem rings)
- N_F_CLASSES 3 → 4 (adds F_SPIRO = 3)
- N_L_CLASSES 5 → 11 (L_MAX+1)
- New per-pair output: spiro_pos_class (8 classes: 0=NO_SPIRO, 1..7
  = position 0..6 in the lower-numbered ring's traversal)
- Pendant tokens removed (was 16 tokens; now zero — moved to A3)
- Token sequence: 6 ring + 15 pair = 21 tokens (was 26)
- New post-processing rules:
  (a) where F != F_SPIRO, force spiro_pos_class = 0
  (b) where F == F_SPIRO but spiro_pos_class == 0, downgrade F to F_NONE
      (otherwise decoder raises)

Inputs at training time (from v5.5 labels in the .pkl):
  R               : (B, R_MAX)            int  ∈ [0, N_R_CLASSES)
  F_mat           : (B, R_MAX, R_MAX)     int  ∈ [0, N_F_CLASSES)
  L_mat           : (B, R_MAX, R_MAX)     int  ∈ [0, N_L_CLASSES)
  spiro_pos_class : (B, R_MAX, R_MAX)     int  ∈ [0, N_SPIRO_POS_CLASSES)
                    (encoder's spiro_atom_positions is shifted by +1; -1
                     becomes 0; see _label_to_spiro_pos_class in
                     run_training_v5_5_a1.py)
  condition       : (B, condition_dim)    float

Outputs at sample time:
  R               : (B, R_MAX)            clean labels (no MASK)
  F               : (B, R_MAX, R_MAX)     symmetric
  L               : (B, R_MAX, R_MAX)     symmetric, zero off-linked
  spiro_pos_class : (B, R_MAX, R_MAX)     symmetric, zero off-spiro

The decoder reads spiro_atom_positions[i, j] only where i < j and
F[i, j] == F_SPIRO (the "anchor" side). The model predicts the upper
triangle; post-processing mirrors to lower.
"""
...
# Single source of truth for all v5.5 capacity / vocab constants.
...
# Max ring size in v5.5 is 7 (Phase 2B added 7-rings), so spiro position
...
# Token-sequence layout (NO pendant tokens in v5.5)
...
# ─── Building blocks (unchanged from v5.4) ─────────────────────────────
...
class AdaLN(nn.Module):
    """LayerNorm with (gamma, beta) predicted from a conditioning vector.

    Zero-init the projection so the block starts as identity (DiT
    convention). This is what makes random-init networks predict close
    to uniform at step 0, which is correct prior behavior for a
...
class RingLayoutDiffusionV5_5(nn.Module):
    """A1 for v5.5: discrete diffusion over (R, F, L, spiro_pos_class).
    ...
    def sample(
        ...
        """Iterative confidence-based unmasking. Same algorithm as v5.4
        but operating on the v5.5 (R, F, L, spiro_pos_class) streams.
        ...
        for step in range(n_steps):
            ...
            # Per-row flat-concat top-K (same trick as v5.4)
        ...
        if post_process:
            out_layout = postprocess_layout_v5_5(out_layout)
        return out_layout


# ─── Post-processing for v5.5 ──────────────────────────────────────────

def postprocess_layout_v5_5(
    layout: Dict[str, torch.Tensor],
) -> Dict[str, torch.Tensor]:
    """Minimal cleanup so the layout is more likely to satisfy the
    v5.5 decoder's structural constraints.
    ...
    those violations cause decode_v5_5_to_scaffold to raise.
    """
    R = layout["R"].clone()
    F_ = layout["F"].clone()
    L = layout["L"].clone()
...
# ─── Capacity presets ──────────────────────────────────────────────────
# Slightly larger defaults than v5.4 — A1 in v5.5 has a harder vocab
...
def build_ring_layout_diffusion_v5_5(
    capacity: str = "3M",
    condition_dim: int = 2,
    cfg_drop_prob: float = 0.1,
    dropout: float = 0.1,
    schedule: str = "cosine",
    **overrides,
) -> RingLayoutDiffusionV5_5:
    """Factory for the A1 v5.5 model at a named capacity.

    Default '3M' (was '1M' in v5.4) — A1 has more outputs in v5.5 and
    ...
    return RingLayoutDiffusionV5_5(
        d_model=cfg["d_model"],
