"""
HiMoFlow v5.5-ZINC — Deterministic scaffold + terminal composition (K=22).

Drop-in replacement for meanflow/compose_full_molecule.py extended for
the ZINC250K terminal vocabulary (K=22) and the v5.5 charged-atom
vocab (atom ids 10-15: O-, N+, n+, N-, n-, P+).

v5.5.1 PATCH (2026-05-13) — fixes for V·U·N validity:

  (1) Charged-atom construction (kills "Element 'N+' not found"
      pre-sanitize crashes). RDKit's Chem.Atom() constructor rejects
      element strings carrying a +/- suffix; you must construct with
      the bare element symbol and apply SetFormalCharge() separately.
      _vocab_id_to_atom() now returns a bare element; _vocab_id_to_charge()
      gives the charge; assemble_molecule() wires both into the RWMol.

  (2) Pre-graft valence + aromatic-degree check (kills the bulk of
      kekulize and valence sanitize failures). Before each terminal
      graft, the assembler now checks:
        - host current explicit valence + attach bond order ≤
          max_valence(element, formal_charge)
        - if host is aromatic, host degree must be ≤ 2 prior to graft
          (sp2 atoms cap at degree 3 total; adding a substituent to a
          fusion/spiro aromatic atom breaks kekulizability)
      Grafts that fail either check are silently skipped — the host
      atom is left with an open valence that RDKit's SanitizeMol fills
      with implicit H. The molecule still sanitizes; we just lose that
      one terminal decoration. Validity recovers; the only cost is mild
      under-decoration for ring atoms Terminal wanted to decorate but
      couldn't fit.

v5.5.2 PATCH (2026-05-13) — Q1+Q2 repair cascade. After failure-dump
analysis showed 120/169 (71%) of residual failures were kekulize errors
and 49 (29%) were valence errors in scaffold-internal bonds (i.e. NOT
caused by terminal grafting — the v5.5.1 graft validator already handles
those), the sanitize step is now a three-stage cascade:

  Attempt 1: vanilla Chem.SanitizeMol(mol).  ~83.5% of molecules
             succeed here. If it works, return immediately.
  Attempt 2 (Q1, aromaticity re-perception): On kekulize failure, clear
             every atom's IsAromatic flag and every bond's BondType (if
             AROMATIC -> SINGLE), then re-sanitize. This lets RDKit
             derive aromaticity from the bond topology rather than from
             the model's per-atom aromatic flags. Principled because
             RDKit, not us, decides which atoms end up aromatic. The
             aromatic flags A2 emitted are treated as a hint, not an
             assertion. If A2 marked a 5-ring of all-c as aromatic and
             that's not kekulizable, the ring becomes a cyclopentadiene
             instead of being rejected.
  Attempt 3 (Q2, bounded valence repair): On valence failure, find each
             over-valent atom and remove its highest-order non-ring
             bond, then re-sanitize. Ring bonds are NEVER removed — that
             would alter the scaffold topology the model emitted. The
             repair is bounded: at most ONE non-ring bond removal per
             over-valent atom, and at most _MAX_REPAIR_PASSES total
             passes. If valence can't be fixed without touching ring
             bonds, the molecule is given up.

The repair cascade is part of the deterministic decoder, not a post-hoc
filter. Validity reported downstream includes repair. This is a defensible
choice — the model's job is to produce a graph that determines a molecule,
and the decoder's job is to produce the molecule consistently. RDKit's
aromaticity perception is part of that pipeline, not an external fix.
...
# ─── Atom vocab (v5.5 with charged-N/P additions) ────────────────────
...
# v5.5.1: map vocab symbol -> (RDKit element, formal_charge).
...
def vocab_id_to_smiles_atom(vid, degree=0):
    ...
    construction. Unchanged from v5.5.0; the RDKit path uses
...
def _vocab_id_to_atom(vid: int) -> Tuple[str, bool]:
    """v5.5.1: returns (bare_element_symbol, is_aromatic).
...
def _vocab_id_to_charge(vid: int) -> int:
    """v5.5.1: formal charge for vocab id (0 for neutral species, ±1 for
...
# v5.5.1: bond-order lookup for pre-graft valence accounting.
...
# v5.5.1: maximum permitted explicit valence per element (neutral form).
...
# v5.5.2: bounded valence repair — max passes per molecule.  A pass
...
def _strip_aromaticity_inplace(mol: Chem.RWMol) -> None:
    """v5.5.2 Q1 helper: clear all aromatic flags on atoms and convert
...
def _find_overvalent_atoms(mol: Chem.RWMol) -> list:
    """v5.5.2 Q2 helper: return list of (atom_idx, current_valence, max_valence)
...
def _try_remove_one_nonring_bond(mol: Chem.RWMol, atom_idx: int) -> bool:
    """v5.5.2 Q2 helper: try removing the highest-order non-ring bond
...
def _repair_and_sanitize(mol: Chem.RWMol) -> Optional[Chem.Mol]:
    """v5.5.2 cascade: vanilla → Q1 aromaticity re-perception → Q2
...
_TERMINAL_SPECS = {
    ...
    # ───── v5.5 additions (model classes 17-22) ─────
...
def assemble_molecule(
    ...
    try:
        ...
        # 1. Scaffold atoms — v5.5.1: handle formal charges
        ...
        # 3. Graft terminals — v5.5.1: valence-aware, skip infeasible grafts
        ...
        # 4. Sanitize — v5.5.2: three-stage cascade
