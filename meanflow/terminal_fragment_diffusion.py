"""
HiMoFlow v5.4 — Stage 2 (terminal_fragment_diffusion), Batch 5.
================================================================

Per-atom categorical over (K+1) classes: 0 = no decoration, 1..K = real
fragments. This is v5.3's design, retrained on the extended terminal
vocab (K=9 instead of K=6) introduced in B2.1.
...
-----------------
   (sol, gap)
       │
       ▼
   A1 (B3) ───► layout (R, F, L, P_len, P_pos)
                       │
                       ▼
        decode_layout_to_scaffold (deterministic)
                       │
                       ▼
        (atom_ids_pad=0, bond_classes,
         atom_mask, aromatic_constraint_mask)
                       │
                       ▼
   A2 (B4) ───► scaffold atom IDs
                       │
                       ▼
   Stage 2 (B5 ◄ this) ──► per-atom fragment IDs
                            (0 = no decoration,
                             1..9 = OH, COOH, NH2, SO3H, F,
                                    CH3, =O, =NH, =S)
                       │
                       ▼
   compose_full_molecule (deterministic, downstream)

ARCHITECTURE
============
v5.3-faithful: per-atom categorical, bond-biased self-attention, single
forward pass, NO cross-attention. Only changes:
  - num_fragments: 6 → 9 (+ =O, =NH, =S)
  - condition_dim: hardcoded 2 → auto-detected (matches A2 convention)
  - bias_enabled flag for ablation (off → standard self-attention)
  - load_v5_3_warmstart() helper for transferring v5.3's checkpoint
    weights into the wider B5 model.

NUM_BOND_CLASSES is intentionally KEPT AT 5 (none/single/aromatic/double/
triple) for clean v5.3 weight transfer. v5.4's scaffold only emits classes
{0, 1, 2}, so slots 3 and 4 of `bond_bias` are never exercised. The cost
is a few hundred unused parameters; the benefit is bitwise-identical
weight transfer for the existing 6 fragments.

VOCAB MAPPING (model class ↔ SMARTS file id)
============================================
  model class 0  = no decoration                (no SMARTS id)
  model class 1  = OH    (SMARTS id 0)
  model class 2  = COOH  (SMARTS id 1)
  model class 3  = NH2   (SMARTS id 2)
  model class 4  = SO3H  (SMARTS id 3)
  model class 5  = F     (SMARTS id 4)
  model class 6  = CH3   (SMARTS id 5)
  model class 7  = =O    (SMARTS id 6)   ← B5 new
  model class 8  = =NH   (SMARTS id 7)   ← B5 new
  model class 9  = =S    (SMARTS id 8)   ← B5 new

So `model_class = SMARTS_id + 1` for any real fragment.
"""
...
# ─── Constants ──────────────────────────────────────────────────────────
# Kept at 5 for v5.3 weight-transfer compatibility. v5.4 scaffolds only
# use {0=none, 1=single, 2=aromatic}; slots 3, 4 are unused at inference
# but retained so saved v5.3 `bond_bias` tensors load with no surgery.
NUM_BOND_CLASSES = 5

# Default atom vocab size (matches ring_atom_diffusion.N_ATOM_CLASSES = 10).
DEFAULT_NUM_ATOM_TYPES = 10

# v5.4 extended terminal vocab (K=9). v5.3 used K=6.
DEFAULT_NUM_FRAGMENTS = 9

# v5.3's vocab for warm-start checks.
...
class AdaLN(nn.Module):
    ...
    Note: same shape as v5.3's AdaLN — single Linear(d_cond, 2*d_model)
    with zero-init weight + bias. Loads v5.3 checkpoints directly.
    """
...
class BondBiasedAttention(nn.Module):
    ...
    Tensor names match v5.3 verbatim (qkv, out_proj, bond_bias) so v5.3
...
class DiTBlock(nn.Module):
    """One transformer block. Shape-compatible with v5.3."""
...
class FragmentStage2(nn.Module):
    """Per-atom fragment classifier. Tensor naming matches v5.3 so state
...
# ════════════════════════════════════════════════════════════════════════
#  v5.3 → v5.4 WARM-START
# ════════════════════════════════════════════════════════════════════════

def load_v5_3_warmstart(
    model: FragmentStage2,
    v5_3_state_dict: Dict[str, torch.Tensor],
    strict_shape_check: bool = True,
    verbose: bool = True,
) -> Dict[str, str]:
    """Load v5.3 weights into a B5 model, expanding fragment-aware tensors.
    ...
    What gets EXPANDED (zero-padded for new classes):
      - frag_input_embed.weight: (8, d) → (11, d)
          v5.3 row 0 (no decoration)  → B5 row 0
          v5.3 rows 1..6 (OH..CH3)    → B5 rows 1..6
          B5 rows 7, 8, 9 (=O,=NH,=S) ← zeros (new)
          v5.3 row 7 (MASK)           → B5 row 10 (MASK)
      - frag_head.weight: (7, d) → (10, d)
          v5.3 rows 0..6              → B5 rows 0..6
...
def build_fragment_stage2(
    ...
    **overrides,
    ...
    v5.3 baseline). For ZINC250K, use '30M'.
