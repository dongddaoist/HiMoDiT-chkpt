"""
HiMoFlow v5.4 — Ring layout decoder (Batch 1.1, with explicit pendant positions).

Pure-Python, no learnable params. Converts the v5.4 layout state
(R, F, L, P_len, P_pos, atom_ids) into v5.3-compatible
(atom_ids_padded, bond_classes_padded, atom_mask) tensors.

This is the foundation that lets v5.4 reuse v5.3's Stage 2 + graft +
matrix-to-SMILES pipeline as-is. The bond matrix is constructed
DETERMINISTICALLY from the layout, eliminating the [6+4] failure
mode by construction.

──── Layout state (5 tensors) ────

R: (R_MAX,) int — ring-type IDs
    0 = <PAD>
    1 = (6, aromatic)   — benzene-class
    2 = (6, aliphatic)  — cyclohexane-class
    3 = (5, aromatic)   — pyrrole/furan/thiophene-class
    4 = (5, aliphatic)  — cyclopentane-class

F: (R_MAX, R_MAX) int — symmetric ring-relation matrix
    0 = no relation
    1 = fused (rings share an edge, 2 atoms in common)
    2 = linked (rings connected by a single bond + optional linker chain)

L: (R_MAX, R_MAX) int — linker chain lengths, only meaningful where F=2
    L[i, j] in [0, L_MAX] = number of -CH2- atoms between rings i and j.

P_len: (R_MAX, P_MAX) int — pendant chain lengths
    P_len[i, p] in [0, P_LEN_MAX] = length of pendant p on ring i.
    P_len[i, p] == 0 → no pendant in this slot.

P_pos: (R_MAX, P_MAX) int — pendant attachment positions on the ring
    P_pos[i, p] in [0, ring_size(i) - 1] when P_len[i, p] > 0.
    P_pos[i, p] == 0 when P_len[i, p] == 0 (PAD value, ignored).

    The position is interpreted relative to the canonical ring traversal
    order produced by build_atom_layout(). Position 0 is the first atom
    of ring i in the traversal.

    Validity constraints:
      - P_pos must NOT collide with fusion-edge positions (those atoms
        are bonded to another ring; pendant would over-saturate them).
      - P_pos must NOT collide with linker-attachment positions.
      - Two pendants on the same ring must NOT share a position.

──── Atom numbering (canonical, deterministic) ────

Atoms are laid out in this order:
  1. Anchor ring (ring 0): atoms 0..size_0-1 in traversal order
  2. For each subsequent ring k (in increasing index):
     If fused: shares 2 atoms with anchor; new (size_k - 2) atoms appended
     If linked: linker has L[anchor, k] new atoms FIRST, then size_k new ring atoms
  3. Pendants: for each ring i, for each pendant p with P_len[i, p] > 0,
     append P_len[i, p] new atoms attached to ring i at position P_pos[i, p].

──── Bond classes (matches v5.3 vocab) ────

Within ring k of type t: aromatic if t aromatic else single.
Fusion edge: inherits aromaticity (must agree between the two rings).
Linker chain: SINGLE.
Pendant chain: SINGLE.

──── Vocab constants (matches v5.3 / config.py) ────
"""
...
# ─── Constants ──────────────────────────────────────────────────────
M_MAX = 40       # v5.4-ZINC Phase 2A applied (was 24)
R_MAX = 6        # v5.4-ZINC Phase 2A applied (was 4)
P_MAX = 6        # v5.4-ZINC Phase 2A applied (was 4)
L_MAX = 10       # v5.4-ZINC Phase 2A applied (was 4)
P_LEN_MAX = 10   # v5.4-ZINC Phase 2A applied (was 5)

# Ring-type IDs
RING_PAD       = 0
# v5.4-ZINC Phase 2B applied: extended ring vocab 5 -> 11 entries
RING_6_AROM    = 1
RING_6_ALIPH   = 2
RING_5_AROM    = 3
RING_5_ALIPH   = 4
...
F_FUSED  = 1
F_LINKED = 2
F_SPIRO  = 3   # v5.5: sp³-quaternary spiro center (one shared atom)

# v5.5 capacity for branched pendants (replaces P_LEN_MAX as the limit-controlling
# constant for pendant trees). Linear-pendant code paths still use P_LEN_MAX so
# legacy v5.4 labels keep working.
B_LEN_MAX = 15   # v5.5: max atoms per branch tree (was P_LEN_MAX=10 linear chain)
# Note: P_MAX remains 6 by default. The branched encoder may use a higher P_MAX,
# but that's parameter-passed at extract time, not a module constant.
...
# Bond class IDs (same as v5.3 / config.py)
BOND_NONE     = 0
BOND_SINGLE   = 1
BOND_AROMATIC = 2

# Atom vocab (same as v5.3): {0:<PAD>, 1:c, 2:O, 3:C, 4:N, 5:n, 6:S, 7:F, 8:s, 9:o}
# v5.4-ZINC Phase 2D applied. Aromatic IDs include n+ (id 12) and n- (id 14).
    ...
    for k in range(1, n_rings):
        ...
        if len(anchors) == 0:
            raise ValueError(
                ...
                "Disconnected ring layouts not supported in v5.4-1."
            )
        if len(anchors) > 1:
            raise ValueError(
                f"Ring {k} has multiple anchors {anchors}. v5.4-1 supports "
...
def decode_layout_to_scaffold(
    ...
    if M_MAX_out is None:
        M_MAX_out = M_MAX  # call-time lookup; robust to monkey-patching
    """Compose the layout + atom IDs into v5.3-compatible scaffold tensors.
    ...
    constraint = aromatic_constraint_mask(R, F, L, P_len, P_pos, M_MAX_out=M_MAX_out)
...
def decode_layout_batch(
    ...
    if M_MAX_out is None:
        M_MAX_out = M_MAX  # call-time lookup; robust to monkey-patching
    """Batch wrapper for decode_layout_to_scaffold."""
    ...
    for b in range(B):
        a, c, m = decode_layout_to_scaffold(
...
# ────────────────────────────────────────────────────────────────────────
# v5.5 decoder: decode_v5_5_to_scaffold
# ────────────────────────────────────────────────────────────────────────
#
# Reconstruct the bond_classes matrix from v5.5 labels. v5.5 differs from
# v5.4 in two ways:
...
def decode_v5_5_to_scaffold(
    ...
    """Decode v5.5 labels back to bond_classes and atom_mask.
    ...
    for k in range(1, n_rings):
        ...
        if relation == F_FUSED:
            ...
            # By v5.4 convention, the fusion atoms in ring k's traversal are
    ...
    for k in range(1, n_rings):
        ...
        # (v5.4 convention: linkers attach at ring atom 0)
...
def aromatic_constraint_mask_v5_5(
    bond_classes: np.ndarray,
    atom_mask: np.ndarray,
) -> np.ndarray:
    """v5.5 aromatic-constraint mask, derived from decoded bond_classes.

    An atom is aromatic-constrained iff it has any incident BOND_AROMATIC
    edge. By construction of decode_v5_5_to_scaffold, only ring atoms in
    ...
    Args:
        bond_classes: (M_MAX, M_MAX) int — output of decode_v5_5_to_scaffold
        atom_mask:    (M_MAX,)        bool — output of decode_v5_5_to_scaffold
