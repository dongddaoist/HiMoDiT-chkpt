"""
HiMoFlow v5.5 — A3 (branch topology) model.
=============================================

One-pass predictor for branched-pendant topology, conditioned on the
ring-layout output of A1 (R, F, L, spiro_atom_positions) and the
property condition.

Outputs per (ring k, slot p) ∈ [0, R_MAX) × [0, P_MAX):
  B_size   ∈ [0, B_LEN_MAX]          (16 classes; class 0 = "no branch")
  B_pos    ∈ [0, MAX_RING_SIZE)      (7 classes; position on the ring)
  B_parent ∈ [0, B_LEN_MAX]          (16 classes; per branch-atom i)
                                       0 = ring root at B_pos
                                       k≥1 = tree atom (k-1)
  B_bond   ∈ [0, N_BOND_CLASSES)     (5 classes; per branch-atom i)
                                       0=NONE, 1=SINGLE, 2=AROMATIC,
                                       3=DOUBLE, 4=TRIPLE

DESIGN DECISIONS (locked 2026-05-11):
  - A3 does NOT predict branch atom IDs. A2 owns all atom_ids in the
    A1 → A3 → A2 → Terminal inference order. At training A2 sees full
    v5.5-decoded bond_classes; at inference same.
  - Sentinel-in-vocab for spiro_pos: -1 in encoder → NO_SPIRO_CLS at
    dataset load time. N_SPIRO_POS_CLASSES = 8 (positions 0-6 + NO_SPIRO).
  - One-pass (non-diffusion) model with flat-MLP heads. CFG via
    cond-dropout. Two-headed-transformer upgrade is future work.
  - No atom_id conditioning. A3 sees only macro-layout (R, F, L,
    spiro_pos) + property condition. Matches inference path.

Architecture
------------
Token sequence (length N_TOKENS = R_MAX + N_PAIR_TOKENS + R_MAX*P_MAX
                              = 6 + 15 + 48 = 69):

  ring tokens [0 .. R_MAX-1]         embed R[k]
  pair tokens [R_MAX .. R_MAX+N_PAIR-1]
                                      embed F[i,j], L[i,j],
                                            spiro_pos[i,j], spiro_pos[j,i]
  slot tokens [...remaining 48]       positional-only; gather context via SA

Each token is sum of role+slot positional embedding + value embeddings
for its categorical fields. DiT stack with AdaLN conditioning on
SiLU(Linear(cond)). Slot tokens are read out and projected through
four heads.

Heads:
  size_head:   Linear(d_model → N_B_SIZE_CLASSES)
  pos_head:    Linear(d_model → N_B_POS_CLASSES)
  parent_head: Linear(d_model → B_LEN_MAX * N_B_PARENT_CLASSES)
  bond_head:   Linear(d_model → B_LEN_MAX * N_B_BOND_CLASSES)

Loss (masked cross-entropy):
  L_size:   over all R_MAX*P_MAX slots (no mask; 0 is a class).
  L_pos:    masked where B_size > 0.
  L_parent: per-atom-index i, masked where i < B_size.
  L_bond:   same mask as L_parent.

Total loss = L_size + L_pos + L_parent + L_bond (equal weights default).

References to layout shapes use the decoder's authoritative constants
(M_MAX=40, R_MAX=6, L_MAX=10, B_LEN_MAX=15) and the encoder's V5_5_P_MAX=8.
"""
...
# neural building blocks; safe to import despite A1's v5.4 constants).
from meanflow.ring_layout_diffusion import AdaLN, DiTBlock

# Authoritative v5.5 constants from the decoder
from meanflow.ring_layout_decoder import (
    R_MAX, L_MAX, B_LEN_MAX,
    F_NONE, F_FUSED, F_LINKED, F_SPIRO,
    RING_PAD, RING_TYPE_INFO,
)

# v5.5 encoder uses V5_5_P_MAX=8 (preprocessing/ring_layout_dataset.py line 1098),
# while the decoder's module-level P_MAX is still 6 (its v5.4 value). Override
# here to match the encoder — the .pkl labels are shaped (R_MAX, P_MAX_v5_5=8).
...
# Ring vocab (Phase 2B): PAD + 10 ring types.
N_R_CLASSES = 11

# F vocab (v5.5): NONE, FUSED, LINKED, SPIRO.
N_F_CLASSES = 4

# L vocab: 0..L_MAX inclusive (linker length).
N_L_CLASSES = L_MAX + 1   # = 11

# Maximum ring size in v5.5 vocab is 7 (Phase 2B).
MAX_RING_SIZE = 7
...
N_SLOT_TOKENS = R_MAX * P_MAX                          # 48 (P_MAX=8 from v5.5 encoder)
...
class BranchTopologyModel(nn.Module):
    """One-pass branch-topology predictor (A3 in v5.5 pipeline).
