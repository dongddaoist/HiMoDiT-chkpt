"""
HiMoFlow v5.5 — Terminal (fragment_stage2) training script.
============================================================

Trains FragmentStage2 on v5.5 labels produced by extract_layout_v5_5().
...
Differences from v5.4 training script:
  ...
  - NAME_TO_SMARTS_ID extended from K=9 → K=22 (full v5.5-ZINC vocab)
...
Use as a script:
    python run_training_v5_5_terminal.py \\
        --labels-pkl ./dataset_labels_v5_5.pkl \\
        --ckpt-dir   ./checkpoints_terminal_v5_5 \\
...
from meanflow.terminal_fragment_diffusion import (
    build_fragment_stage2, count_parameters, compute_class_weights,
    load_v5_3_warmstart,
...
# v5.5 terminal vocab: K=22 (was K=9 in v5.4 RedDB; expanded for ZINC).
# DEFAULT_NUM_FRAGMENTS in terminal_fragment_diffusion.py is 9; we override
# at every call site below. Model output dim = V5_5_NUM_FRAGMENTS + 1
# (class 0 = "no decoration").
V5_5_NUM_FRAGMENTS = 22

# v5.5 atom vocab: K=16 (was K=10 in v5.3/v5.4 RedDB; Phase 2D added 6 charged
...
def _filter_batch_to_loss_inputs(batch: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if missing:
        raise KeyError(
            f"batch missing required keys for compute_loss: {missing}")
    return out
...
class EMA:
    """Same EMA used by A2's training script."""
    ...
    def restore_to(self, model: nn.Module,
...
def cosine_lr_with_warmup(step, warmup_steps, total_steps, base_lr,
                           min_lr_frac=0.1):
    if step < warmup_steps:
        return base_lr * (step + 1) / max(warmup_steps, 1)
    progress = min((step - warmup_steps) / max(total_steps - warmup_steps, 1),
                    1.0)
...
class TerminalDataset(Dataset):
    ...
    by the B5 patch to extract_layout). Falls back gracefully with a
    clear error if old labels are passed in.
    """
    # SMARTS id → model class (lookup; we store the inverse map for
    # named-fragment validation). v5.5-ZINC: full K=22 vocab. The first 9
    # entries (OH..=S) are the v5.4 RedDB names; entries 9-21 added in
    # v5.4-ZINC and v5.4 Phase 2C ("2D-C"). Indices match
    ...
    def __init__(self, labels: List[Dict[str, Any]],
                 num_fragments: int = V5_5_NUM_FRAGMENTS):
        # Sanity-check v5.5 schema: B_* fields and spiro_atom_positions
        # must be present (these replace v5.4's P_len/P_pos)
        if labels:
        ...
        for lab in labels:
            ...
            if "host_canonical_idx" not in t0:
                raise KeyError(
                    ...
                    "Re-run extract_dataset() with the B5-patched "
...
def _collate(items: List[Dict[str, torch.Tensor]]) -> Dict[str, torch.Tensor]:
    return {k: torch.stack([it[k] for it in items], dim=0)
            for k in items[0]}
...
def _parse_args():
    p = argparse.ArgumentParser(description="Train HiMoFlow v5.5 Terminal stage.")
    ...
    p.add_argument("--num-fragments", type=int, default=V5_5_NUM_FRAGMENTS,
                    help="K (default 22 for v5.5-ZINC; 9 for v5.4 extended; "
                         "6 for v5.3 vocab)")
    ...
    p.add_argument("--init-from", type=str, default=None,
                    help="Warm-start from a v5.3 best_model.pt")
