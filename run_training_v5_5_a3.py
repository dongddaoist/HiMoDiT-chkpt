"""
HiMoFlow v5.5 — A3 (branch topology) training script.
======================================================

Trains the BranchTopologyModel on v5.5 labels produced by
extract_layout_v5_5() / extract_dataset_v5_5(). A3 predicts per-(ring,
...
Mirrors run_training_v5_5_a1.py conventions:
...
Use as a script:
    python run_training_v5_5_a3.py \
        --labels-pkl /path/to/ring_layout_labels_v5_5.pkl \
        --ckpt-dir /path/to/checkpoints_v5_5_a3 \
        --epochs 100 \
        --batch-size 256 \
        --capacity 10M
...
from meanflow.branch_topology_diffusion import (
    BranchTopologyModel,
...
from meanflow.ring_layout_decoder import R_MAX, P_MAX, B_LEN_MAX, RING_PAD
...
def _filter_batch_to_loss_inputs(batch: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if missing:
        raise KeyError(
            ...
            f"Verify the .pkl was produced by extract_layout_v5_5() "
            f"(not extract_layout v5.4)."
        )
    return out


# ─── EMA (copied verbatim from run_training_v5_5_a1.py) ──────────────
...
class BranchTopologyDataset(Dataset):
    """Wraps the v5.5 label list. Each __getitem__ loads:
    ...
    def __init__(self, labels: List[Dict[str, Any]]):
        # Sanity-check that labels are v5.5 (have B_size etc.)
        if labels:
...
def evaluate_structural_validity(
    ...
    """
    model.eval()
    from meanflow.ring_layout_decoder import RING_TYPE_INFO
...
def train_a3(
    ...
    if not isinstance(labels, list):
        # extract_dataset writes a list; accept either {"labels": [...]}.
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for batch in train_loader:
            ...
            inputs = _filter_batch_to_loss_inputs(batch)
        ...
        if do_val:
            ...
            with torch.no_grad():
                for vbatch in val_loader:
                    ...
                    vinputs = _filter_batch_to_loss_inputs(vbatch)
...
def _parse_args():
    p = argparse.ArgumentParser(
        description="Train HiMoFlow v5.5 A3 (branch topology) model.",
    )
